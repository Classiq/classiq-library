...
@qfunc
def foo(arr: CArray[CReal], qb: QBit):
    if_(arr.len > 2, lambda: RX(arr[-1], qb), lambda: RX(arr[0], qb))


@qfunc
def main(q0: Output[QBit]):
    allocate(q0)
    foo([0.5, 1.0, 1.5], q0)
```
</Tab>
...
@qfunc
def foo(ms: MyStruct, qv: QArray[QBit, 2]):
    H(qv[0])
    repeat(
...
[sparse Hamiltonian](/sdk-reference/qmod/classical-types#sparsepauliop) objects.
Calling a [Pauli](/sdk-reference/qmod/classical-types#pauli)
