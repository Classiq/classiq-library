...
[quantum](/qmod-reference/language-reference/quantum-types) types, and quantum variables have states that can be a
...
values.
For example, if `x` is a [classical variable](/qmod-reference/language-reference/classical-variables)
...
Similarly, if `qarr` is a [quantum variable](/qmod-reference/language-reference/quantum-variables)
...
-   Array Slice: _array_ **[** _start-index_ **:** _stop-index_ **]**
    ...
    -   In Python, if _array_ is a Python list, use the alternative syntax: **slice_(** _array_ **,** _start-index_ **,** _stop-index_ **)**
