...
The values are interpreted according to their specified types (see [Quantum Types](/qmod-reference/language-reference/quantum-types)).
...
@qfunc
def main(a: Output[QBit], b: Output[QNum[2, UNSIGNED, 2]]):
    allocate(a)
    H(a)
    allocate(b)
...
![parsed_results.png](/qmod-reference/language-reference/resources/parsed_results.png)
