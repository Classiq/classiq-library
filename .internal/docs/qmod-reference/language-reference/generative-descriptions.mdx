...
def foo(qa: QArray[QBit]):
    for i in range(qa.len):  # 'qa.len' is a Python integer
        PHASE(
            math.asin(i / qa.len), qa[i]
        )  # the expression `i / qa.len` is a Python float
```
...
types (for more on classical types, see [Classical Types](/qmod-reference/language-reference/classical-types)). Using
...
(see [Operators](/qmod-reference/language-reference/operators)). In these cases too, parameters may be Python built-in types.
...
-   The attributes of quantum variables - `size`, `len`, `is_signed`, and `fraction_digits` -
    are treated as Python (non-symbolic) expressions (see [Quantum Types](/qmod-reference/language-reference/quantum-types)).
...
-   The index variable in a Qmod `repeat` statement is a Qmod symbolic variable of type
    `CInt` (see [Classical Control Flow](/qmod-reference/language-reference/statements/classical-control-flow)).
-   Execution parameters (i.e., parameters of function `main`) must be symbolic (see
    [Quantum Entry Point](/qmod-reference/language-reference/quantum-entry-point)).
...
def main(q: Output[QBit]):
    allocate(q)
    my_operator(lambda ratio, target: RX(asin(ratio * 2), target), q)
```
